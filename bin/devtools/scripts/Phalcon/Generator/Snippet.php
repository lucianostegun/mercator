...
EOD;
     ...
     * @return \$this
     */
    public function set%s(\$%s, \$force=false){
...
EOD;
        ...
        $modifiedColumnComparation = ($type == 'timestamp' ? "\$this->formatDate(\$this->$fieldName, 'Y-m-d H:i:s') !== \$this->formatDate(\$$fieldName, 'Y-m-d H:i:s')" : "(string)\$this->$fieldName !== (string)\$$fieldName");
    
    switch ($type) {
      case 'integer':
        $fieldValue = "(\$$fieldName !== null ? (int)\$$fieldName : null)";
        break;
      case 'float':
      case 'double':
        $fieldValue = "(\$$fieldName !== null ? (float)\$$fieldName : null)";
        break;
      case 'boolean':
        $fieldValue = "\$$fieldName ? (bool)\$$fieldName : false";
        break;
      case 'date':
      case 'timestamp':
        $fieldValue = "\$this->formatDate(\$$fieldName, 'Y-m-d H:i:s')";
        break;
      default:
        $fieldValue = "\$$fieldName || \$force ? \$$fieldName : null";
        break;
    }
        return PHP_EOL.sprintf($templateSetter, $fieldName, $type, $fieldName, $setterName, $fieldName, $modifiedColumnComparation, $fieldName, $fieldName, $fieldValue).PHP_EOL;
    }
...
EOD;
    ...
    public function getClassBase($namespace, $useDefinition, $classDoc = '', $abstract = '', $className, $extends = '', $content, $license = '', $primaryKeys=array())
    {
        $templateCode = <<<EOD
<?php
...
%s%s%s%s%sclass %sBase extends %s {

  protected \$_primaryKeys = %s;
%s
}
EOD;
    $primaryKeyList = (empty($primaryKeys) ? 'array()' : "array('".implode('\', \'', $primaryKeys)."')");
        return sprintf($templateCode, $license, $namespace, $useDefinition, $classDoc, $abstract, $className, $extends, $primaryKeyList, $content).PHP_EOL;
    }

    public function getClass($namespace, $useDefinition, $classDoc = '', $abstract = '', $className, $extends = '', $license = '')
    {
        $templateCode = <<<EOD
<?php

%s%s%s%s%sclass %s extends %sBase {

}
EOD;
        return sprintf($templateCode, $license, $namespace, $useDefinition, $classDoc, $abstract, $className, $extends).PHP_EOL;
    }
...
EOD;
    ...
    public function getGetter($fieldName, $type, $getterName)
    {
        ...
        switch ($type) {
          case 'timestamp':
            $param1 = "\$format = 'Y-m-d H:i:s'";
            break;
          case 'boolean':
            $param1 = "\$forceNull = false";
            break;
          default:
            $param1 = null;
            break;
...
EOD;
        $fieldValue = ($type == 'timestamp' ? "\$this->formatDate(parent::$getterName(), \$format)" : $fieldName);
        $fieldValue = ($type == 'timestamp' ? "formatDate(\$this->$fieldName, \$format)" : $fieldName);
        $cast       = ($type == 'boolean' ? '$forceNull && $this->'.($fieldName).' === null ? null : (bool)' : null);

        return PHP_EOL.sprintf($templateGetter, $fieldName, $type, $getterName, $param1, $cast, $fieldValue).PHP_EOL;
    }
